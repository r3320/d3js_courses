...
# camera - идентификатор камеры (webcam1, webcam2, screen, ipcam)
...
{
    local video_file="$1"
    local camera1_file="${OUTPUT_DIR%/}/$2-${video_file}"
    local camera2_file="${OUTPUT_DIR%/}/$3-${video_file}"
    local camera3_file="${OUTPUT_DIR%/}/$4-${video_file}"
    local output_file="${OUTPUT_DIR%/}/${video_file}"
    if [ $# -eq 2 ]; then
        mv "${camera1_file}" "${output_file}"
    fi
    if [ $# -eq 4 ]; then
        ffmpeg_exec \
            -i "${camera1_file}" \
...
# генерирование меток в формате: timestamp:duration:group
generate_marks()
{
    ...
    do
        ...
        video_group=$(echo ${video_file} | grep -o -e "\(${VIDEO_GROUPS// /\\|}\)")
        echo ${timestamp}:1:${video_group}
        echo $((timestamp+duration)):-1:${video_group}
...
encode_video_session()
{
    ...
    local output_file="$1.webm"
    # создать временную директорию
    OUTPUT_DIR=$(mktemp -d)
    # перекодировать видео по заданному формату
    ls "${STORAGE_DIR}" | grep -e "^[0-9]\+_\(${VIDEO_GROUPS// /\\|}\)-${session_id}" | while read video_file
    do
    ...
    generate_marks | fragments_by_groups | write_fragments "${output_file}"
    # объединить видеофрагменты по группам
    local video_group
    for video_group in ${VIDEO_GROUPS}
    do
        concat_video_group "${video_group}-${output_file}"
    done
...
# получение списка сессий, архивирование видеофрагментов и загрузка на сервер
archiver()
{
    ls "${STORAGE_DIR}" | 
    grep -e "^[0-9]\+_\(${VIDEO_GROUPS// /\\|}\)-[0-9a-f]\{24\}.*\.webm$" | 
    cut -f2 -d- | sort -u |
    while read video_file
    do
        session_id="${video_file%*.webm}"
        video_file="${session_id}.webm"
...
# перекодирование видеофайлов и объединение их по группам в один экран
VIDEO_GROUPS="webcam1 webcam2 screen" archiver

# перекодирование видеофайлов с ip-камер
VIDEO_GROUPS="ipcam" archiver
